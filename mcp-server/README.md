...
```bash
# 전역 설정 (모든 프로젝트에서 사용)
claude mcp add hwpx --scope user npx -y hwpx-mcp-server

# 프로젝트별 설정
claude mcp add hwpx npx -y hwpx-mcp-server
```
...
`skills/hwpx-document-writer.md` 스킬을 설치하면 AI가 `build_document` 도구를 활용하여 서식이 갖춰진 한글 문서를 **한 번의 호출로** 작성합니다.
