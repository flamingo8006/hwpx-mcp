# hwpx-mcp 설치 패키지 (DEPRECATED — `.mcpb` 로 대체)

> ⚠️ **이 `.sh`/`.bat`/`.ps1` 인스톨러는 은퇴했습니다 (2026-06-02).** 배포는 원클릭 **`.mcpb`(MCP Bundle)** 로 일원화됐으며, `.mcpb` 가 서버 + 템플릿 2종(공문서·업무추진비류 서식)을 자가배포합니다. 이 디렉터리는 참조용 보존본입니다. 빌더 `scripts/build-installer.sh` 도 함께 제거됨. 배포 가이드는 볼트 `AI로_한글문서_자동작성_가이드_mcpb원클릭` 참조.
