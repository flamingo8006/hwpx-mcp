...
1. 모드 판정(아래 "모드 선택" 표) 후, 필요한 템플릿 파일명을 결정 (`공문서_프레임.hwpx` / `업무추진비류_집행내역서_서식.hwpx`).
...
> **역질문 원칙 (Mode A 전용)**: 서식이 고정된 폼필류 문서(현재 `업무추진비류_집행내역서_서식` 1종, 향후 추가될 수 있음)는 **필수 입력값이 없으면 진행하지 않고 역질문**한다. 회계·결재 문서라 빈칸·추정값이 허용되지 않기 때문이다.
> **Mode B·C 는 역질문하지 않는다** — 누락 시 자동 결정 후 그대로 진행 (기존 철학 유지).
...
5. `사용자`(회의 진행자)가 명시되지 않았으면, 대화·참석자 명단에서 **맨 처음 등장한 이름**을 사용자로 추정해 "사용자(회의 진행자)는 ○○ 맞나요?"처럼 **확인 질문**한다 (빈 질문 대신 추정값 제시 → 사용자가 정정 가능). 단정해서 그냥 진행하지는 말 것.

필드 3계층(필수 입력 / 자동 파생 / 보강 생성) 정의·계산 규칙·보강 경계는 **`REFERENCE.md` 의 "Mode A placeholder" 섹션** 참조 필수.

### Step A-0.5 — 작성 내용 확인 (open 전 필수, Mode A 전용)

필수값을 다 확보하면 바로 쓰지 말고, 채울 내용을 한 장으로 정리해 **확인을 1회 받는다.** `총원`·`1인금액`·`금액한글` 은 Claude 가 계산하는 값이라 집계·표기 오류가 결재 문서에 그대로 박힌다 — open 전에 사람이 한 번 검산하게 한다. (A-0 은 "빠진 *입력값* 받기", A-0.5 는 "Claude 의 *계산·해석* 확인"으로 목적이 다르다 — 같은 걸 두 번 묻는 게 아니다.)

아래는 **형식 틀**이다. `<…>` 자리에 이번 작업의 실제 값을 채워 보여준다 (출처 열로 입력 echo / 🧮 계산 / ✍️ 다듬음을 구분해 사용자가 어디를 검산해야 하는지 드러낸다 — `저장 위치` 같은 자동 결정 메타 행은 `—`):

> **작성 내용 확인** — 이대로 한글 문서를 만들까요?
>
> | 항목 | 값 | 출처 |
> |---|---|---|
> | 부서 / 작성자(회의 진행자) / 일자 / 장소 | `<입력값 그대로>` | 입력 |
> | 목적 | `<다듬은 한 줄>` | ✍️ 다듬음 |
> | 금액 | `<총액>원` | 입력 |
> | 금액(한글) | `<금…원 정식표기>` | 🧮 계산 |
...
**규칙**:
- **확인 전 `open_document` 금지** (A-0 의 "필수값 확보 전 open 금지" 게이트의 한 단계 뒤).
- 참석자는 소속별로 묶어 이름을 나열하고, `1인금액` 의 나눗셈(`<총액> ÷ <N>`)을 **식째로** 보여줘 머릿수 오집계가 눈에 드러나게 한다.
- 사용자가 값을 고치면 **영향받는 계산값을 다시 계산**한다 (참석자·금액이 바뀌면 `총원`·`1인금액`·`금액한글` 재계산). `총원`·`금액` 이 바뀐 경우에만 카드를 다시 보여주고, 그 외 소소한 문구 수정은 반영 후 바로 진행한다 (재확인 반복 금지).
...
되묻지 말고 자동 결정 후 저장 메시지에 최종 경로 포함. (단, 이는 **경로·파일명 한정**이다 — Mode A 의 필수 입력값이 누락된 경우에는 Step A-0 역질문 규칙이 우선한다.)
...
1. 올바른 `공문서_프레임.hwpx` 를 열었는지 재확인 (`$PATHS.templates` 의 일치 엔트리 경로)
2. 파일이 손상/수정됐다면 사용자에게 `<templates_dir>` 의 템플릿 재설치 안내
