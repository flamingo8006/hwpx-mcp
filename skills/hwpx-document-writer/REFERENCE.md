...
집행내역서는 **3단계로 채운다**:
- **A. `batch_replace`** — 아래 **8개** placeholder 를 **전부** 치환 (참석자·회의내용 제외). **절대 일부만 치환하고 끝내지 말 것.**
- **B. `update_table_cell`** (참석자) — 참석자 명단은 인원·소속이 가변이라 placeholder 가 아니라 셀 전체 재작성 (아래 "B. 참석자 블록").
- **C. `update_table_cell`** (회의내용) — 회의내용도 안건 수가 가변이라 셀 전체 재작성 (아래 "C. 회의내용 블록").
...
| **보강 생성** | `목적`, 회의내용(가변 N줄) | 사용자가 준 키워드·메모를 **결재 문서체 한 줄로 다듬기** (아래 경계 준수). 회의내용은 안건 수만큼 줄 추가 (아래 "C. 회의내용 블록") |
...
### A. batch_replace placeholder 표 (8개 — 참석자·회의내용 제외)
...
> 회의내용(`{{회의내용1}}`·`{{회의내용2}}`)은 더 이상 placeholder 치환으로 처리하지 않는다 — 줄 수 가변이라 **C. 회의내용 블록**의 `update_table_cell` 로 셀 전체를 다시 쓴다.
...
### C. 회의내용 블록 — `update_table_cell` (가변 줄)

회의내용은 안건 수가 가변이라 `batch_replace` 가 아니라 `update_table_cell` 로 **셀 전체를 다시 쓴다** (템플릿의 고정 `{{회의내용1}}`·`{{회의내용2}}` 2줄 제약을 해소 — 1줄도 3줄도 가능). 좌표는 템플릿 고정값:

```json
{
  "doc_id": "...",
  "section_index": 0, "table_index": 0, "row": 6, "col": 0,
  "char_shape_id": 12,
  "auto_hanging_indent": false,
  "text": "6. 회 의 내 용 :\n - 2026년 AI 교육 로드맵 초안 검토\n - 과정별 강사 섭외 계획 논의\n - 예산 배정 협의"
}
```

**작성 규칙**:
- **첫 줄은 반드시** `6. 회 의 내 용 :` 라벨(공백까지 글자 그대로)로 시작 — 셀 전체를 덮어쓰므로 라벨이 빠지면 사라진다.
- **안건 1건당 한 줄**, 각 줄 앞에 `" - "`(공백+하이픈+공백). 줄 수 **1~N 자유**. 줄 구분은 `\n`.
- 내용은 사용자 메모·키워드를 결재 문서체 한 줄로 다듬되, **없던 안건을 지어내지 말 것**(보강 경계 §위 동일).
- **`char_shape_id: 12` 필수** (미지정 시 폰트 깨짐).
- **`auto_hanging_indent: false` 필수** ("6." 라벨·"-" 불릿의 자동 내어쓰기 개입 방지).

> 참석자(B, row 4)와 회의내용(C, row 6)은 **둘 다 같은 표(table 0)의 다른 행**이라 `update_table_cell` 을 각각 1회씩 호출한다(서로 간섭 없음).
