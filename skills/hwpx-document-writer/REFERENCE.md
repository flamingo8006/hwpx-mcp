...
`batch_replace({ doc_id, replacements: [ ... ] })` 호출에 아래 14개 placeholder 를 **전부** 포함한다. **절대 일부만 치환하고 끝내지 말 것.**
...
| 계층 | 필드 | 처리 |
|------|------|------|
| **필수 입력** | `부서`, `사용자`, `일자`, `장소`, `금액`, `참석자수`, `참석자1~N` | 대화에 없으면 **Step A-0 역질문**. 추측·임의값·기본값 금지. **`부서`도 기본값(정보전산팀) 없음 — 반드시 확인.** |
| **자동 파생** | `금액한글`, `1인금액` | 사용자 입력 불필요 — Claude 가 `금액`·`참석자수` 에서 계산 (아래 규칙) |
| **보강 생성** | `목적`, `회의내용1`, `회의내용2` | 사용자가 준 키워드·메모를 **결재 문서체 한 줄로 다듬기** (아래 경계 준수) |

### 자동 파생 계산 규칙

- `금액한글` = `금액`(숫자)의 한글 표기. 끝에 `원` 포함. 예: `84,000` → `팔만사천원`.
- `1인금액` = `금액` ÷ `참석자수`, **원 단위 반올림**, 쉼표 포함. 예: `84,000 ÷ 4 = 21,000`. 나누어떨어지지 않으면 반올림 후 표기.
...
| `{{참석자수}}` | 참석자 총 인원 (숫자) | 필수 | `4` |
| `{{참석자1}}` | 참석자 이름 1 | 필수 | `홍길동` |
| `{{참석자2}}` | 참석자 이름 2 | 필수 | `김철수` |
| `{{참석자3}}` | 참석자 이름 3 | 필수 | `이영희` |
| `{{금액한글}}` | 총액 (한글) | 파생 | `팔만사천원` |
| `{{금액}}` | 총액 (숫자, 쉼표 포함) | 필수 | `84,000` |
| `{{1인금액}}` | 1인당 환산 금액 | 파생 | `21,000` |
| `{{회의내용1}}` | 회의 요약 1줄 | 보강 | `2026년 AI 교육 로드맵 초안 검토` |
| `{{회의내용2}}` | 회의 요약 1줄 | 보강 | `과정별 강사 섭외 계획 논의` |

**참석자 4명 이상**: 템플릿엔 이름 필드가 3개뿐이다. `참석자수` 에 총원을 정확히 넣고, 4번째부터는 `참석자3` 을 `이영희 외 2명` 형태로 묶거나 `회의내용` 에 풀어 기재한다. Step A-0 에서 **총원과 전체 이름을 모두 받아둘 것.**
